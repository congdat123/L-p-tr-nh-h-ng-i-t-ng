<template>
  <div class="container">
   ...
   <div class="form null" v-if="this.listCart.length < 1" >
    <p style="margin: 120px; text-align: center;" >Giỏ hàng trống!.</p>
   </div>
   
 <div class="row cart" v-if="this.listCart.length > 0">
  <div class="col-md-12 order-md-12 mb-12">
    <h4 class="d-flex justify-content-between align-items-center mb-3">
      <span class="text-muted">Giỏ hàng</span>
      <span class="badge badge-secondary badge-pill">{{countCart}}</span>
    </h4>
    <table class="table" v-if="this.listCart.length  > 0">
      <thead>
        <tr >
          <th scope="col">#</th>
...
<div id="form" v-if="this.listCart.length > 0">
...
methods: {
  ...
  async AddOder() {      
    ...
    if( this.countCart === 0  ) {  
      ...
      const retVal = confirm("Bạn có muốn đặt hàng không?");
      if( retVal === true ){
        try {   
